
// Modular curves downloaded from the LMFDB on 09 October 2025.
// Magma code for modular curve with label 48.96.3.nk.1

// Other names and/or labels
// Cummins-Pauli label: 8B3
// Rouse-Sutherland-Zureick-Brown label: 48.96.3.1213

// Group data
level := 48;
// Elements that, together with Gamma(level), generate the group
gens := [[7, 23, 12, 29], [19, 6, 26, 37], [25, 4, 40, 17], [25, 11, 28, 7]];
// Group contains -1?
ContainsMinus1 := true;
// Index in Gamma(1)
index := 96;

// Curve data
conductor := [[2, 22], [3, 2]];
bad_primes := [2, 3];
// Genus
g := 3;
// Rank
r := 1
;// Exact gonality known
gamma := 2;

// Modular data
// Number of cusps
Ncusps := 12
;// Number of rational cusps
Nrat_cusps := 0
;// CM discriminants
CM_discs := [];
// Groups containing given group, corresponding to curves covered by given curve
covers := ["16.48.2.bh.1", "24.48.1.gy.1", "48.48.0.ca.2", "48.48.1.dl.1", "48.48.1.dn.1", "48.48.2.bh.1", "48.48.2.cb.1"];

// Models for this modular curve, if computed
Pol<x,y,z,w,t,u> := PolynomialRing(Rationals(), 6);
// Isomorphic to P^1?
is_P1 := false;

// Embedded model
model_0 := [z*t-y*u,y*z-y*w-y*t-w*u+t*u,2*w*t+2*t^2+y*u-u^2,2*y*z+y*w+y*t-z*u-w*u+t*u,z^2-2*z*w-2*w^2+2*t^2,3*y^2-2*w*t+2*t^2-y*u,24*x^2+z^2+z*w+w^2];

// Singular plane model
model_1 := [144*x^8+336*x^6*y^2+136*x^4*y^4+20*x^2*y^6+y^8-576*x^6*z^2-1200*x^4*y^2*z^2-240*x^2*y^4*z^2-12*y^6*z^2+1008*x^4*z^4+1080*x^2*y^2*z^4+126*y^4*z^4-864*x^2*z^6-216*y^2*z^6+324*z^8];

// Weierstrass model
model_2 := [582*x^8+3408*x^7*z+8736*x^6*z^2+12768*x^5*z^3+11760*x^4*z^4+6720*x^3*z^5+2688*x^2*z^6+384*x*z^7+y^2+96*z^8];

// Maps from this modular curve, if computed

// j-invariant map from the embedded model
//   Coordinate number 0:
map_0_coord_0 := -2^7*(3857868*y*t^10*u-30894534*y*t^8*u^3+92354094*y*t^6*u^5-137935791*y*t^4*u^7+106283214*y*t^2*u^9-28391571*y*u^11-1137240*z*w^11-24786*z*w^7*u^4+276696*z*w^5*u^6-859005*z*w^3*u^8+2776860*z*w*u^10-832518*w^12+390258*w^8*u^4-813564*w^6*u^6+2351241*w^4*u^8-7580322*w^2*u^10+832572*t^12-14970528*t^10*u^2+69808086*t^8*u^4-149432848*t^6*u^6+169269576*t^4*u^8-92448000*t^2*u^10+16368787*u^12);
//   Coordinate number 1:
map_0_coord_1 := 1*(18*y*t^2*u^9-9*y*u^11-36*z*w^3*u^8+12*z*w*u^10-27*w^4*u^8+12*w^2*u^10-4*t^12+32*t^10*u^2-104*t^8*u^4+176*t^6*u^6-139*t^4*u^8+40*t^2*u^10-5*u^12);

// Map from the embedded model to the plane model of modular curve with label 48.96.3.nk.1
//   Coordinate number 0:
map_1_coord_0 := 1*(x);
//   Coordinate number 1:
map_1_coord_1 := 1*(1/2*w);
//   Coordinate number 2:
map_1_coord_2 := 1*(1/6*u);
// Codomain equation:
map_1_codomain := [144*x^8+336*x^6*y^2+136*x^4*y^4+20*x^2*y^6+y^8-576*x^6*z^2-1200*x^4*y^2*z^2-240*x^2*y^4*z^2-12*y^6*z^2+1008*x^4*z^4+1080*x^2*y^2*z^4+126*y^4*z^4-864*x^2*z^6-216*y^2*z^6+324*z^8];

// Map from the embedded model to the Weierstrass model of modular curve with label 48.96.3.nk.1
//   Coordinate number 0:
map_2_coord_0 := 1*(-3/4*w*t-3/4*t^2-1/4*t*u+1/2*u^2);
//   Coordinate number 1:
map_2_coord_1 := 1*(-81/8*x*w*t^3*u^3-81/8*x*w*t^2*u^4-81/8*x*t^4*u^3-27/2*x*t^3*u^4+135/16*x*t^2*u^5+513/64*x*t*u^6);
//   Coordinate number 2:
map_2_coord_2 := 1*(3/4*w*t+3/4*t^2-1/4*t*u-5/8*u^2);
// Codomain equation:
map_2_codomain := [582*x^8+3408*x^7*z+8736*x^6*z^2+12768*x^5*z^3+11760*x^4*z^4+6720*x^3*z^5+2688*x^2*z^6+384*x*z^7+y^2+96*z^8];
