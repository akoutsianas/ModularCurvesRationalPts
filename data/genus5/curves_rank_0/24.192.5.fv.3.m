
// Modular curves downloaded from the LMFDB on 16 October 2025.
// Magma code for modular curve with label 24.192.5.fv.3

// Other names and/or labels
// Cummins-Pauli label: 24Z5
// Rouse-Sutherland-Zureick-Brown label: 24.192.5.1569

// Group data
level := 24;
// Elements that, together with Gamma(level), generate the group
gens := [[17, 3, 12, 19], [19, 22, 0, 13], [23, 19, 12, 23], [23, 22, 0, 23]];
// Group contains -1?
ContainsMinus1 := true;
// Index in Gamma(1)
index := 192;

// Curve data
conductor := [[2, 24], [3, 7]];
bad_primes := [2, 3];
// Genus
g := 5;
// Rank
r := 0
;// Exact gonality known
gamma := 4;

// Modular data
// Number of cusps
Ncusps := 24
;// Number of rational cusps
Nrat_cusps := 0
;// CM discriminants
CM_discs := [];
// Groups containing given group, corresponding to curves covered by given curve
covers := ["24.96.1.dc.1", "24.96.1.dm.2", "24.96.1.dp.4", "24.96.3.fy.1", "24.96.3.gc.1", "24.96.3.gt.1", "24.96.3.gw.3"];

// Models for this modular curve, if computed
Pol<x,y,z,w,t> := PolynomialRing(Rationals(), 5);
// Isomorphic to P^1?
is_P1 := false;

// Canonical model
model_0 := [3*x^2+2*x*y-y^2-z^2,3*x^2-4*x*y-y^2-z^2-w^2,6*z^2+4*w^2+t^2];

// Singular plane model
model_1 := [784*x^8+120*x^6*y^2+9*x^4*y^4+1120*x^6*z^2+168*x^4*y^2*z^2-776*x^4*z^4-90*x^2*y^2*z^4-840*x^2*z^6+441*z^8];

// Maps from this modular curve, if computed

// j-invariant map from the canonical model
//   Coordinate number 0:
map_0_coord_0 := -1*((2*w^2+t^2)^3*(559104*y^2*w^16-104448*y^2*w^14*t^2-376320*y^2*w^12*t^4-2970624*y^2*w^10*t^6-3236736*y^2*w^8*t^8-1642368*y^2*w^6*t^10-463008*y^2*w^4*t^12-69888*y^2*w^2*t^14-4368*y^2*t^16-280064*w^18-40704*w^16*t^2-158976*w^14*t^4-502656*w^12*t^6-425472*w^10*t^8-166464*w^8*t^10-31248*w^6*t^12-2328*w^4*t^14-18*w^2*t^16-t^18));
//   Coordinate number 1:
map_0_coord_1 := 1*(t^2*w^8*(4*w^2+t^2)*(96*y^2*w^10-48*y^2*w^8*t^2-264*y^2*w^6*t^4-204*y^2*w^4*t^6-60*y^2*w^2*t^8-6*y^2*t^10+16*w^12-12*w^10*t^2+141*w^8*t^4+136*w^6*t^6+57*w^4*t^8+12*w^2*t^10+t^12));

// Map from the canonical model to the plane model of modular curve with label 24.192.5.fv.3
//   Coordinate number 0:
map_1_coord_0 := 1*(x);
//   Coordinate number 1:
map_1_coord_1 := 1*(2/3*z+2/3*t);
//   Coordinate number 2:
map_1_coord_2 := 1*(1/3*w);
// Codomain equation:
map_1_codomain := [784*x^8+120*x^6*y^2+9*x^4*y^4+1120*x^6*z^2+168*x^4*y^2*z^2-776*x^4*z^4-90*x^2*y^2*z^4-840*x^2*z^6+441*z^8];
