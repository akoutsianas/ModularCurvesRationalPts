
// Modular curves downloaded from the LMFDB on 16 October 2025.
// Magma code for modular curve with label 24.144.5.tx.1

// Other names and/or labels
// Cummins-Pauli label: 24W5
// Rouse-Sutherland-Zureick-Brown label: 24.144.5.900

// Group data
level := 24;
// Elements that, together with Gamma(level), generate the group
gens := [[1, 19, 16, 11], [5, 14, 22, 23], [13, 4, 20, 13], [13, 22, 20, 1]];
// Group contains -1?
ContainsMinus1 := true;
// Index in Gamma(1)
index := 144;

// Curve data
conductor := [[2, 24], [3, 7]];
bad_primes := [2, 3];
// Genus
g := 5;
// Rank
r := 1
;// Exact gonality known
gamma := 4;

// Modular data
// Number of cusps
Ncusps := 8
;// Number of rational cusps
Nrat_cusps := 0
;// CM discriminants
CM_discs := [];
// Groups containing given group, corresponding to curves covered by given curve
covers := ["24.72.1.dz.1", "24.72.2.cz.1", "24.72.2.ed.1", "24.72.2.fd.1", "24.72.2.fs.1", "24.72.3.bdc.1", "24.72.3.bdt.1"];

// Models for this modular curve, if computed
Pol<x,y,z,w,t> := PolynomialRing(Rationals(), 5);
// Isomorphic to P^1?
is_P1 := false;

// Canonical model
model_0 := [2*x*z-x*w+y*z+y*w,x^2+x*y+y^2+z^2+2*z*w-2*w^2,3*x*y+5*z^2-2*z*w+2*w^2+6*t^2];

// Singular plane model
model_1 := [x^8+15*x^4*y^4+16*x^4*y^2*z^2+x^4*z^4-18*x^2*y^6-30*x^2*y^4*z^2-6*x^2*y^2*z^4+6*x^2*z^6+9*y^8+30*y^6*z^2+37*y^4*z^4+20*y^2*z^6+4*z^8];

// Maps from this modular curve, if computed

// Map from the canonical model to the canonical model of modular curve with label 24.72.3.bdc.1
//   Coordinate number 0:
map_0_coord_0 := 1*(2*t);
//   Coordinate number 1:
map_0_coord_1 := 1*(-z+2*w-t);
//   Coordinate number 2:
map_0_coord_2 := 1*(-x+y);
// Codomain equation:
map_0_codomain := [7*x^4+11*x^3*y+15*x^2*y^2+8*x*y^3+4*y^4-9*x^2*z^2-6*x*y*z^2-6*y^2*z^2+3*z^4];

// Map from the canonical model to the plane model of modular curve with label 24.144.5.tx.1
//   Coordinate number 0:
map_1_coord_0 := 1*(x);
//   Coordinate number 1:
map_1_coord_1 := 1*(z);
//   Coordinate number 2:
map_1_coord_2 := 1*(t);
// Codomain equation:
map_1_codomain := [x^8+15*x^4*y^4+16*x^4*y^2*z^2+x^4*z^4-18*x^2*y^6-30*x^2*y^4*z^2-6*x^2*y^2*z^4+6*x^2*z^6+9*y^8+30*y^6*z^2+37*y^4*z^4+20*y^2*z^6+4*z^8];
