
// Modular curves downloaded from the LMFDB on 16 October 2025.
// Magma code for modular curve with label 24.144.5.ks.1

// Other names and/or labels
// Cummins-Pauli label: 12D5
// Rouse-Sutherland-Zureick-Brown label: 24.144.5.156

// Group data
level := 24;
// Elements that, together with Gamma(level), generate the group
gens := [[3, 14, 14, 9], [3, 19, 10, 21], [15, 1, 10, 9], [17, 18, 0, 1], [21, 22, 22, 15]];
// Group contains -1?
ContainsMinus1 := true;
// Index in Gamma(1)
index := 144;

// Curve data
conductor := [[2, 28], [3, 8]];
bad_primes := [2, 3];
// Genus
g := 5;
// Rank
r := 2
;// Exact gonality known
gamma := 4;

// Modular data
// Number of cusps
Ncusps := 12
;// Number of rational cusps
Nrat_cusps := 2
;// CM discriminants
CM_discs := [];
// Groups containing given group, corresponding to curves covered by given curve
covers := ["12.72.1.u.1", "24.72.3.zo.1", "24.72.3.zq.1"];

// Models for this modular curve, if computed
Pol<x,y,z,w,t> := PolynomialRing(Rationals(), 5);
// Isomorphic to P^1?
is_P1 := false;

// Canonical model
model_0 := [y*t+z*w+z*t,y^2-2*y*z-2*z^2+w^2+t^2,3*x^2+y^2+y*z-y*w+y*t+z^2+w*t];

// Singular plane model
model_1 := [9*x^4*y^4+18*x^4*y^2*z^2+9*x^4*z^4+36*x^2*y^6+36*x^2*y^5*z-24*x^2*y^4*z^2-24*x^2*y^3*z^3-12*x^2*y^2*z^4-12*x^2*y*z^5+9*y^8-24*y^6*z^2+6*y^4*z^4+8*y^2*z^6+z^8];

// Maps from this modular curve, if computed

// Map from the canonical model to the canonical model of modular curve with label 24.72.3.zo.1
//   Coordinate number 0:
map_0_coord_0 := 1*(-x);
//   Coordinate number 1:
map_0_coord_1 := 1*(x+w+t);
//   Coordinate number 2:
map_0_coord_2 := 1*(x+y+2*z);
// Codomain equation:
map_0_codomain := [14*x^4-2*x^3*y+4*x^2*y^2-x*y^3-2*x^3*z-2*x^2*y*z+5*x*y^2*z-y^3*z+4*x^2*z^2+5*x*y*z^2+4*y^2*z^2-x*z^3-y*z^3];

// Map from the canonical model to the plane model of modular curve with label 24.144.5.ks.1
//   Coordinate number 0:
map_1_coord_0 := 1*(x);
//   Coordinate number 1:
map_1_coord_1 := 1*(y+z);
//   Coordinate number 2:
map_1_coord_2 := 1*(w);
// Codomain equation:
map_1_codomain := [9*x^4*y^4+18*x^4*y^2*z^2+9*x^4*z^4+36*x^2*y^6+36*x^2*y^5*z-24*x^2*y^4*z^2-24*x^2*y^3*z^3-12*x^2*y^2*z^4-12*x^2*y*z^5+9*y^8-24*y^6*z^2+6*y^4*z^4+8*y^2*z^6+z^8];
